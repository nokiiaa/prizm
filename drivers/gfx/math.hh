...
struct Rect
{
	...
	inline Rect(int left, int right, int top, int bottom) :
		Left(left), Right(right), Top(top), Bottom(bottom) { }

	inline int Width() const { return Right - Left; }
	inline int Height() const { return Bottom - Top; }

	inline Rect ClipInto(const Rect &rect) const
	{
		int left = Math::Clamp(Left, rect.Left, rect.Right);
		int right = Math::Clamp(Right, rect.Left, rect.Right);
		int top = Math::Clamp(Top, rect.Top, rect.Bottom);
		int bottom = Math::Clamp(Bottom, rect.Top, rect.Bottom);
...
struct Color
{
	...
	inline Color Alphablend(Color src)
	{
		if (A == 0 || src.A == 255) return src;
		if (src.A == 0) return *this;

		int c = src.A;
		int d = A * (255 - src.A) / 256;
		int div = c + d;
		...
		return Color(
			((src.R * c + R * d) / 256) * 256 / div,
			((src.G * c + G * d) / 256) * 256 / div,
			((src.B * c + B * d) / 256) * 256 / div,
			div
		);
	}
